...
public class FacilitiesMaintenanceRequestImpl
    implements IDatabaseAPI<FacilitiesMaintenanceRequest, Integer> {

  private static final FacilitiesMaintenanceRequestImpl instance =
      new FacilitiesMaintenanceRequestImpl();
  private final String TABLE_NAME = "FACILITIES_SERVICE";
  private Connection connection = null;
  private Statement statement = null;
  private ArrayList<FacilitiesMaintenanceRequest> facilitiesServiceList = null;
  private ServiceRequestImpl serviceRequestImpl = null;

  /** Private constructor that populates ArrayList */
  private FacilitiesMaintenanceRequestImpl() {
    this.facilitiesServiceList = new ArrayList<>();
    this.statement = Ddb.getStatement();
    this.connection = Ddb.getConnection();
    this.serviceRequestImpl = ServiceRequestImpl.getInstance();

    this.tableOnStartup("csv/FacilitiesMaintenance.csv");
  }
   ...
   */
  public static FacilitiesMaintenanceRequestImpl getInstance() {
    return instance;
  }
      ...
      final PreparedStatement CREATE =
          this.connection.prepareStatement(
              "CREATE TABLE FACILITIES_SERVICE("
                  ...
                  + "CONSTRAINT fk_facilitiesServiceID FOREIGN KEY (serviceID) REFERENCES SERVICE_REQUEST(SERVICEID) "
                  + "ON DELETE CASCADE"
                  + ")");
  ...
  public void populateFromList() throws SQLException {
    ...
    for (FacilitiesMaintenanceRequest request : this.facilitiesServiceList) {
      final PreparedStatement UPDATE =
          this.connection.prepareStatement("INSERT INTO FACILITIES_SERVICE VALUES(?)");
      ...
      final PreparedStatement JOIN =
          this.connection.prepareStatement(
              "SELECT *\n"
                  ...
                  + "    ON FACILITIES_SERVICE.SERVICEID = SERVICE_REQUEST.SERVICEID");

      final ResultSet selection = JOIN.executeQuery();

      while (selection.next()) {
        final FacilitiesMaintenanceRequest request =
        ...
        this.facilitiesServiceList.add(request);
  ...
  public void storeToCSV(File filename) throws FileNotFoundException {
    CSVUtil.generateCSV(filename, this.facilitiesServiceList);
  }
  ...
  public void restoreFromCSV(String filename) throws IOException, SQLException {
    ...
    while (buffer.hasNextLine()) {
      ...
      this.facilitiesServiceList.add(
          new FacilitiesMaintenanceRequest(
  ...
  public void deleteTuple(Integer primaryKey) throws SQLException {
    this.facilitiesServiceList.removeIf(request -> request.getServiceID() == primaryKey);
  }

  @Override
  public void addNewEntity(FacilitiesMaintenanceRequest entity) throws SQLException {
    final PreparedStatement UPDATE =
        this.connection.prepareStatement("INSERT INTO FACILITIES_SERVICE VALUES(?)");
    ...
    this.facilitiesServiceList.add(entity);

    UPDATE.close();
  }

  /**
   * Get all instances of an FacilitiesMaintenace
   *
   * @return ArrayList of FacilitiesMaintenance
   */
  @Override
  public ArrayList<FacilitiesMaintenanceRequest> getAll() {
    return this.facilitiesServiceList;
  }
  ...
  public void modifyEntity(Integer serviceID, String field, Object newValue) throws SQLException {
    final PreparedStatement INSERT =
        this.connection.prepareStatement(
            "UPDATE FACILITIES_SERVICE SET " + field + "=? WHERE SERVICEID=?");
  ...
  public void clearTable() throws SQLException {
    final PreparedStatement WIPE =
        this.connection.prepareStatement("DELETE FROM FACILITIES_SERVICE");
    WIPE.executeUpdate();
    WIPE.close();
  }
}
